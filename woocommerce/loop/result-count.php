...
<p class="woocommerce-result-count" <?php echo (empty($orderedby) || 1 === intval($total)) ? '' : 'role="alert" aria-relevant="all" data-is-sorted-by="true"'; ?>>
	...
	if (1 === intval($total)) {
		...
		printf(_n('Showing <span> %1$d </span> item', 'Showing <span> %1$d </span> items', $total, 'woocommerce') . $orderedby_placeholder, $total, esc_html($orderedby));
	} else {
