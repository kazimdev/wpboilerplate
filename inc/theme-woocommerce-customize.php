...
if (! class_exists('WPBoilerplate_Woocomerce_Customize')) {
	class WPBoilerplate_Woocomerce_Customize
	{
		//$instance variable
		...
		function add_product_delivery_info()
		{
			...
			$delivery_date = wpboilerplate_get_business_day($delivery_days);
...
?>
		...
		function woocommerce_header_add_to_cart_fragment($fragments)
		{
			...
			ob_start();
		?>
			<a class="wpboilerplate-header-cart" href="<?php echo wc_get_cart_url(); ?>"
				title="<?php esc_attr_e('View your shopping cart', 'wpboilerplate'); ?>">
				<i class="fa fa-shopping-basket" aria-hidden="true"></i>
				<span class="cart-badge"><?php echo sprintf('%d', WC()->cart->get_cart_contents_count()); ?></span>
			</a>
		<?php
			$fragments['a.wpboilerplate-header-cart'] = ob_get_clean();
		...
		{
			$class[] = is_product() ? 'wpboilerplate-product-single-page-item' : 'wpboilerplate-single-product-item';
		...
		{
			?>
				<ul class="wpboilerplate-thumb-inner-item-list">
				<?php
			}
			...
			public function woocommerce_before_shop_loop_item_li_add_to_cart()
			{
				?>
					<li>
						...
						global $product;
						echo apply_filters(
							'wpboilerplate_woocommerce_loop_add_to_cart_link', // WPCS: XSS ok.
							sprintf(
								...
								wp_kses($args['icon'], WPBoilerplate()->kses_allowed_html('all'))
							),
							$product,
							$args
						);
...
<?php
		...
		if (class_exists('WPBoilerplate_Woocomerce_Customize')) {
			WPBoilerplate_Woocomerce_Customize::getInstance();
		}
	}
