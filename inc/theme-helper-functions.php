...
if (!class_exists('WPBoilerplate_Helper_Functions')) {
    ...
    class WPBoilerplate_Helper_Functions
    {
        /**
         * $instance
         * @since 1.0.0
        ...
        function post_thumbnail($size = 'wpboilerplate_classic')
        {
...
?>
        ...
        public function link_pages()
        {
            ...
            $defaults = array(
                'before' => '<div class="wp-link-pages"><span>' . esc_html__('Pages:', 'wpboilerplate') . '</span>',
                'after' => '</div>',
                'link_before' => '',
                'link_after' => '',
        ...
        {
            global $wp_query;
            $allowed_html = wpboilerplate()->kses_allowed_html('all');
            $big = 12345678;
            if (null == $nav_query) {
                ...
                if (is_array($page_format)) {
                    ...
                    echo '<li><span>' . esc_html($paged) . esc_html__(' of ', 'wpboilerplate') . esc_html($wp_query->max_num_pages) . '</span></li>';
                ...
                if (is_array($page_format)) {
                    ...
                    echo '<li><span>' . esc_html($paged) . esc_html__(' of ', 'wpboilerplate') . esc_html($nav_query->max_num_pages) . '</span></li>';
                    foreach ($page_format as $page) {
                        echo "<li>" . wp_kses($page, $allowed_html) . "</li>";
                    }
                    print '</ul></div>';
        ...
        public function posted_by()
        {
            $byline = sprintf(
                ...
                esc_html_x(' %s', 'post author', 'wpboilerplate'),
                '<a class="post-by url fn n" href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '"> ' . esc_html__('By ', 'wpboilerplate') . esc_html(get_the_author()) . '</a>'
            );
        ...
        public function posted_on()
        {
            ...
            $posted_on = sprintf(
                ...
                esc_html_x(' %s', 'post date', 'wpboilerplate'),
        ...
        public function author_avatar($post_id)
        {
            ...
            printf('<div class="post-author"><div class="author-image"><img src="%1$s" class="image-fit rounded-circle" alt="%3$s"></div><a href="%2$s"> ' . esc_html__("By", "wpboilerplate") . ' %3$s </a></div>', $author_image, $author_link, $author_name);
        }
        ...
        public function author_biography($post_id)
        {
            ...
            printf('<div class="post-author"><div class="author-image"><img src="%1$s" class="image-fit" alt="%3$s"></div><div class="author-content"><cite class="post-by">' . esc_html__('Written By', 'wpboilerplate') . '</cite><h3 class="title">%2$s</h3><p>%4$s</p></div></div>', $author_image, $author_name, $author_name, $author_desc);
        }
        ...
        public function posted_tag()
        {
            ...
            $tags_list = get_the_tag_list('', esc_html_x(' ', 'list item separator', 'wpboilerplate'));
            if ($tags_list) {
                    ...
                    <div class="prev-post">
                        <a href="<?php echo esc_url(get_permalink($prev_post->ID)); ?>">
                            <div class="title-with-link">
                                <span><?php esc_html_e('Prev Post', 'wpboilerplate') ?></span>
                ...
                <?php if (!empty($next_post)): ?>
                    <div class="next-post">
                        <a href="<?php echo esc_url(get_permalink($next_post->ID)); ?>">
                            <div class="title-with-link">
                                <span><?php esc_html_e('Next Post', 'wpboilerplate') ?></span>
        ...
        public function post_navigation_with_img()
        {
            ...
            if (!empty(get_previous_post_link())) {
                ...
                $output .= '<div class="content"><span class="prev-post">' . esc_html__('Previous', 'wpboilerplate') . '</span>';
            ...
            if (!empty(get_next_post_link())) {
                ...
                $output .= '<div class="content"><span class="next-post">' . esc_html__('Next', 'wpboilerplate') . '</span>';
                        ...
                        <div class="product-related-wrap">
                            <ul class="product-related-posts colum-<?php echo esc_attr($post_details['posts_per_page']) ?>">
                                ...
                                while ($query->have_posts()) : $query->the_post();
                                    ...
                                    $img_url_val = $img_id ? wp_get_attachment_image_src($img_id, 'wpboilerplate_grid', false) : '';
                                        ...
                                        <div class="service-single-item-04 margin-bottom-10">
                                            ...
                                            <div class="content-wrap post-<?php echo esc_attr($post_details['post_type']) ?>">
                                                <div class="content">
                                                    ...
                                                    <?php WPBoilerplate_Excerpt('18') ?>
                                                    <a class="read-btn"
                                                        href="<?php the_permalink(); ?>"><?php echo esc_html__('Read More', 'wpboilerplate') ?>
...
<?php
        ...
        public function page_id()
        {
            ...
            $page_type_id = (isset($post->ID) && is_array(self::get_pages_id()) && in_array($post->ID, self::get_pages_id())) ? $post->ID : false;
        ...
        /**
         * Is wpboilerplate active
         * @since 1.0.0
         */
        public function is_wpboilerplate_active()
        {
            $theme_name_array = array('WPBoilerplate', 'WPBoilerplate Child');
        ...
        /**
         * is wpboilerplate core active
         * @since 1.0.0
         * */
        public function is_wpboilerplate_core_active()
        {
            return defined('WPBOILERPLATE_CORE_SELF_PATH');
        }
        ...
        {
            $comments_count = get_comments_number(get_the_ID());
            $comment_text = ($comments_count > 1) ? esc_html__('Comments', 'wpboilerplate') . ' (' . $comments_count . ')' : esc_html__('Comments', 'wpboilerplate') . ' (' . $comments_count . ')';
